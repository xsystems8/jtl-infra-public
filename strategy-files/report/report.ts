...
import { ReportChart, ReportChartOptions } from './report-chart';
import { ReportCard } from './report-card';
import { TradingViewChart, TradingViewMultipointShapes, TradingViewShapes } from './report-tv';
import { debugLog, error, log, getLogs, warningOnce } from '../log';
...
export class Report extends BaseObject {
  ...
  storeState = async (key: string) => {
    if (this.tables['Previous logs']) {
      delete this.tables['Previous logs'];
    }
    this.tableUpdate('Previous logs', [...getLogs('error'), ...getLogs('log'), ...getLogs('trace')]);
    await global.storage.storeState(key, this);
  };
   ...
   *
   */
  cardSetValue(cardName: string, value: number | string | boolean, aggType: AggType = 'last') {
  ...
  tableUpdate(tableName: string, data: TableRow[] | TableRow, idField: string = 'id') {
    ...
    // } catch (e) {
    //   throw new BaseError('report.tableUpdate ' + e.message, { e, tableName });
    // }
  }
  ...
  addActionButton(title: string, action: string, value: string) {
    ...
    if (this.actionButtons) {
      for (const button in this.actionButtons) {
        try {
          ...
          error('report.updateReport(actionButtons)', e.message, { e });
        }
      }
    }
