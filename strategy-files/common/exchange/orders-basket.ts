...
export class OrdersBasket extends BaseObject {
  ...
  private readonly params: OrdersBasketParams;
  private isHedged: boolean;

  protected onOpenPositionCallBack: Function;
  protected onClosePositionCallBack: Function;

  _ordersById: Map<string, Order>;
  _historyOrdersById: Map<string, Order>;
  orderNextId = 0;
  ...
  log = (event: string, msg: string, params?: Record<string, any>) => {
    if (this.isLoggingEnabled) {
      log(this.orderNextId + ' | ' + event, msg, params);
    }
  };

  getOrderById = (ownOrderId: string) => {
  ...
  registerOnOpenPosition(callback: Function) {
    this.onOpenPositionCallBack = callback;
  }
  registerOnClosePosition(callback: Function) {
  ...
  // TODO: TYPE params
  modifyOrder = async (ownOrderId: string, params) => {};

  buyMarket = async (size: number, sl = 0, tp = 0, params: Record<string, any> = {}) => {
  ...
  sellMarket = async (size, sl = 0, tp = 0, params: Record<string, any> = {}) => {
  ...
  buyLimit = async (size, price, sl = 0, tp = 0, params: Record<string, any> = {}) => {
  ...
  sellLimit = async (size, price, sl = 0, tp = 0, params: Record<string, any> = {}) => {
  ...
  sellStopLimit = async (size, price, stopPrice, sl = 0, tp = 0, params: Record<string, any> = {}) => {
  ...
  buyStopLimit = async (size, price, stopPrice, sl = 0, tp = 0, params: Record<string, any> = {}) => {
  ...
  _createOwmOrder = async (type, side, amount, price, params: Record<string, any> = {}): Promise<Order> => {
    let ownId = 'O';
  ...
  _createOrder = async (
    ...
    // TODO: не хватает symbol
    const symbol = '';
    order = await createOrder(symbol, type, side, amount, price, order_params);
  ...
  async onOpenPosition(ownPos, ownOrder) {
  ...
  async onClosePosition(ownPos, ownOrder) {
  ...
  createStopOrders = async (ownOrder) => {
    ...
    let tpSide: OrderSide = ownOrder.side === 'buy' ? 'sell' : 'buy';
    let slSide: OrderSide = ownOrder.side === 'buy' ? 'sell' : 'buy';
